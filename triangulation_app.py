...
from utils import triangulate

class TriangulationApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Триангуляция маслёнка")  # Устанавливаем заголовок окна
        ...
        self.device = Device(50, 50)  # Устройство
    ...
    def init_ui(self):
        ...
        self.calc_button = tk.Button(self.root, text="Докс (найти устройство)", command=self.calculate_coordinates)
        self.calc_button.grid(row=4, column=0, columnspan=4)

        self.update_tower_params_button = tk.Button(self.root, text="Обновить вышки", command=self.update_tower_params)
    ...
    def redraw(self):
        ...
        # Рисуем башни
        for i, tower in enumerate(self.towers):
            self.draw_circle(tower.x, tower.y, tower.r, f"Вышка {i + 1}")
    ...
    def draw_circle(self, x, y, r, label):
        ...
        self.canvas.create_oval(x - r, self.canvas_size - (y - r), x + r, self.canvas_size - (y + r), outline="blue", tags="tower")
        self.canvas.create_text(x, self.canvas_size - y, text=label, fill="black")

    def draw_device(self, x, y):
        # Рисуем устройство
        x, y = x * self.scale, y * self.scale
        self.canvas.create_oval(x - 5, self.canvas_size - (y - 5), x + 5, self.canvas_size - (y + 5), fill="red", tags="device", outline="black")
        self.canvas.tag_raise("device")  # Убедиться, что устройство находится на верхнем слое
        self.canvas.create_text(x + 10, self.canvas_size - y, text="Устройство", fill="black")
    ...
    def update_tower_params(self):
        ...
        try:
            ...
            messagebox.showerror("Введи нормальные значения", "Неправильный ввод")

    def calculate_coordinates(self):
        # Расчет координат устройства
        try:
            if self.device.is_inside_all_towers(self.towers):
                x_calc, y_calc = triangulate(self.towers)
                messagebox.showinfo("Координаты", f"Вычесленные корды:\nX: {x_calc:.2f}, Y: {y_calc:.2f}")
            else:
                messagebox.showwarning("Вне зоны", "Устройство вне зоны трёх вышек")
        except ValueError:
            messagebox.showerror("Ошибо4ка", "Введённые значения не верны")
